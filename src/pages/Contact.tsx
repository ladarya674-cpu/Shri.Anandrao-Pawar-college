...
export default function Contact() {
  ...
  const handleEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val && !isGmail(val)) {
      setEmailError('Only Gmail addresses are accepted (e.g. yourname@gmail.com)');
    } else {
      setEmailError('');
    }
  };
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNameError(val && !isValidName(val) ? 'Enter letters and spaces only.' : '');
  };

  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value.replace(/\D/g, '').slice(0, 10);
    setFormData({ ...formData, phone: val });
    setPhoneError(val && !isValidPhone(val) ? 'Enter exactly 10 digits only.' : '');
  };
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      {/* Banner */}
      <div className="py-8 sm:py-10 px-4 text-white border-b-4 border-amber-500" style={{ backgroundColor: '#1e3764' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-blue-100 text-xs sm:text-sm mt-2 max-w-2xl leading-relaxed">
            Reach out to our admissions cell, principal&apos;s office, or visit our campus in
            Chiplun.
          </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-5 space-y-5">
            <div>
              ...
              <p className="text-slate-600 text-xs sm:text-sm leading-relaxed mb-5">
                Our administrative staff and admission coordinators are available during college
                working hours to assist students, parents, and alumni.
              </p>
              <div className="flex flex-wrap gap-2">
                <a href="mailto:shriram.society@rediffmail.com" className="inline-flex items-center gap-2 rounded-lg bg-[#1e3764] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[#0d2444] transition-colors">
                  <span aria-hidden="true">✉</span> Email the office
                </a>
                <a href="https://www.google.com/maps/search/?api=1&query=Kanganewadi%20Khend%20Chiplun" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 hover:border-[#e07b00] hover:text-[#c06800] transition-colors">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 shadow-sm">
              <h3 className="text-sm font-bold text-[#1e3764] mb-4" style={{ fontFamily: 'Fraunces, Georgia, serif' }}>
                Visit or write to us
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs text-slate-600">
                <div>
                  <p className="font-bold uppercase tracking-wider text-[10px] text-[#e07b00] mb-1.5">Campus address</p>
                ...
                <div className="space-y-4">
                  <div>
                    <p className="font-bold uppercase tracking-wider text-[10px] text-[#e07b00] mb-1.5">Office hours</p>
                    <p>Monday – Saturday, 9:00 AM – 5:00 PM</p>
                    <p>Sunday &amp; state holidays: Closed</p>
                  </div>
                  <div>
                    <p className="font-bold uppercase tracking-wider text-[10px] text-[#e07b00] mb-1.5">Email</p>
                    <a href="mailto:shriram.society@rediffmail.com" className="font-semibold text-[#1e3764] break-words hover:text-[#e07b00]">shriram.society@rediffmail.com</a>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 lg:sticky lg:top-28">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm">
              ...
              <p className="text-xs sm:text-sm text-slate-500 mb-1">
                Have questions regarding course admissions, fee structures, or document
                verification? Submit your details below.
              </p>
              {/* Gmail notice */}
              <div className="flex items-center gap-2 mb-5 mt-2 px-3 py-2 rounded-xl bg-blue-50 border border-blue-100 text-xs text-blue-700 font-medium">
                <span>📧</span>
                Only <strong className="mx-1">Gmail addresses</strong> (@gmail.com) are accepted for
                enquiries.
              </div>
              ...
              {formSent ? (
                <div className="bg-emerald-50 border border-emerald-300 rounded-2xl p-6 text-center text-emerald-800 space-y-2 animate-fade-in">
                  ...
                  <p className="text-xs text-emerald-700">
                    Thank you for reaching out. Your enquiry has been recorded and the college
                    administrative desk will respond to your Gmail shortly.
                  </p>
                  <button
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent ${nameError ? 'border-red-400 focus:ring-red-400 bg-red-50' : 'border-slate-300 focus:ring-[#e07b00]'}`}
                    />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        maxLength={120}
                        className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent transition-all ${emailError
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:border-transparent ${phoneError ? 'border-red-400 focus:ring-red-400 bg-red-50' : 'border-slate-300 focus:ring-[#e07b00]'}`}
                      />
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!submitting) e.currentTarget.style.backgroundColor = '#c06800';
                    }}
                    onMouseLeave={(e) => {
