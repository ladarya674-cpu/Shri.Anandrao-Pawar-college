  ...
  useEffect(() => {
    const fetchNotices = async () => {
      ...
      try {
        ...
        setError('Could not load notices. The backend server may not be running.');
        console.error(e);
      } finally {
        setLoading(false);
      }
      ...
      <div className="py-8 sm:py-10 px-4 text-white border-b-4 border-amber-500" style={{ backgroundColor: '#1e3764' }}>
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-blue-100 text-xs sm:text-sm mt-2 max-w-2xl">
            Stay updated with current circulars, academic notifications, examination schedules, and
        ...
        {error && (
          <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl p-4 text-amber-800 text-sm">
            ...
            <span className="block text-xs mt-1 text-amber-600">
              Run <code className="bg-amber-100 px-1 rounded font-mono">node backend/server.js</code> to start the backend.
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-5">
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all ${isSelected
                          ? 'text-white font-semibold shadow-sm'
                          : 'text-slate-700 hover:bg-slate-100'
                        }`}
                      style={{ backgroundColor: isSelected ? '#1e3764' : 'transparent' }}
                    >
                      <span>{cat}</span>
                    </button>
                  );
            ...
            <div className="rounded-xl p-5 border border-amber-200 bg-amber-50/70 text-slate-800">
              <h4 className="font-bold text-sm text-[#1e3764] mb-1">Administrative Help Desk</h4>
              <p className="text-xs text-slate-600 leading-relaxed mb-3">
                For official circular clarification or certificate verification, contact the college
                office.
              </p>
          ...
          <div className="lg:col-span-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4 px-1 pb-3 border-b border-slate-200">
              <div>
                <p className="text-[10px] uppercase tracking-wider font-bold text-[#e07b00] mb-0.5">Official updates</p>
                <div className="text-xs sm:text-sm font-semibold text-slate-600">
                Official notices in <span className="text-[#e07b00]">{selectedCategory}</span>
                </div>
              </div>
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
                ...
                <h3 className="text-base font-bold text-slate-700 mb-1">No Notices Found</h3>
                <p className="text-xs sm:text-sm text-slate-500 max-w-md mx-auto">
                  No circulars matched your search query. Try different keywords or select "All
                  Notices".
                </p>
                <button
                  onClick={() => {
              ...
              <div className="space-y-3.5">
                {filtered.map((n) => (
                  <div
                    ...
                    className={`bg-white border rounded-2xl p-4 sm:p-5 hover:shadow-md transition-all duration-200 cursor-pointer group flex flex-col sm:flex-row items-start gap-4 ${n.important
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-[10px] sm:text-xs uppercase font-semibold text-amber-200 leading-none mt-1">
                        {new Date(n.uploadedAt).toLocaleString('en-IN', { month: 'short' })}
                      </span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-3 flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                        ...
                        <span className="text-slate-400">{formatDate(n.uploadedAt)}</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          >
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              {activeNoticeModal.content && (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 text-xs sm:text-sm text-slate-700 leading-relaxed">
                  <p className="font-semibold text-slate-800 mb-2">Notice Circular Summary:</p>
              ...
              <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 text-xs space-y-1.5">
                ...
                <p className="text-slate-600">
                  📅 Uploaded on: <span className="font-semibold text-slate-800">{formatDateTime(activeNoticeModal.uploadedAt)}</span>
                </p>
                {activeNoticeModal.originalName && (
                  <p className="text-slate-600">
                    📄 File: <span className="font-semibold text-slate-800">{activeNoticeModal.originalName}</span>
                    {activeNoticeModal.fileSize && (
                      <span className="text-slate-500 ml-1">({activeNoticeModal.fileSize})</span>
                    )}
                  </p>
                )}
                {!activeNoticeModal.filename && (
                  <p className="text-slate-500 italic">No PDF attached to this notice.</p>
                )}
              </div>

              <div className="p-4 rounded-xl border border-amber-200 bg-amber-50 text-xs text-slate-700 space-y-1">
                <p className="font-bold text-[#1e3764]">Instructions for Students & Faculty:</p>
                <p>• All official college communications published here are signed by the Principal / Registrar.</p>
                <p>• For physical verification or duplicate copies, visit the college administrative office during working hours (9:00 AM - 5:00 PM).</p>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-100 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <span className="text-xs text-slate-500">
                Doc Ref: SAP/NOT/{activeNoticeModal.id.slice(0, 8).toUpperCase()}/2026
              </span>
