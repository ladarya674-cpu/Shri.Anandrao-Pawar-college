...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 shadow-md">
      {/* 1. Utility Top Bar */}
      <div
        className="text-white text-xs py-1.5 px-3 sm:px-4"
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center gap-2">
          <div className="flex items-center gap-2 sm:gap-4 overflow-hidden text-[11px] sm:text-xs">
            <span className="truncate sm:hidden">
              Established 2018&nbsp; | &nbsp;Kanganewadi, Khend, Chiplun
            </span>
            <span className="hidden sm:inline truncate">
              📍 Kanganewadi, Khend, Chiplun, Dist-Ratnagiri
            </span>
      ...
      <div
        className="px-3 sm:px-4 py-2 sm:py-3.5 border-b border-white/10"
      ...
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-6">
          ...
          <Link
            to="/"
            className="flex items-center gap-2.5 sm:gap-4 min-w-0 flex-1 group"
          >
            <img
              src={logo}
            ...
            <div>
              <p
                className="text-[9px] sm:text-xs font-semibold tracking-wider uppercase mb-0.5"
                style={{ color: "#f9a825" }}
              >
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-6 flex-shrink-0">
            ...
            <div className="flex flex-col items-center">
              <div className="flex flex-col items-center justify-center w-12 h-12 lg:w-14 lg:h-14 rounded-full border-2 border-amber-400 bg-amber-400/10">
                <span className="text-amber-300 font-bold text-[10px] lg:text-xs leading-none">
                  NAAC
                </span>
                <span className="text-white font-extrabold text-sm lg:text-base leading-none mt-0.5">
                  C
                </span>
              </div>
              <span className="text-slate-300 text-[10px] mt-1 font-medium">
                Accredited
              </span>
            </div>
      ...
      >
            ...
            {navLinks.map(({ to, label }) => (
              <li key={to} className="h-full">
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center h-full px-4 text-xs xl:text-sm font-semibold transition-all border-b-4 -mb-1 ${
                      isActive
        ...
        {open && (
          <div
            className="lg:hidden border-t border-white/15 px-3 sm:px-4 py-3 animate-fade-in shadow-2xl"
            style={{ backgroundColor: "#1e3764" }}
          >
            <div className="flex items-center justify-around py-2 px-3 rounded-xl bg-white/5 border border-white/10 mb-3 text-center text-xs">
              <div>
                <span className="text-amber-400 font-bold block">NAAC C</span>
