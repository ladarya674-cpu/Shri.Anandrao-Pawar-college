...
export default function Footer() {
  return (
    <footer style={{ backgroundColor: '#0d2444' }} className="text-slate-300 border-t-4 border-amber-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              />
              <div>
                <p className="text-[10px] uppercase font-bold tracking-wider" style={{ color: '#f9a825' }}>
                  Shriram Education Society's
                </p>
                <span className="text-white font-bold text-sm sm:text-base leading-tight block" style={{ fontFamily: 'Lora, Georgia, serif' }}>
                  Shri. Anandrao Pawar Mahavidyalaya
                </span>
              </div>
            </div>
            <p className="text-xs sm:text-sm leading-relaxed text-slate-400 mb-4">
              Arts, Commerce & Science Mahavidyalaya, Kanganewadi, Khend, Chiplun. Affiliated to University of Mumbai and Accredited by NAAC.
            </p>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-amber-300 text-xs font-semibold">
              <span>⭐</span> NAAC 'A' Grade Accredited
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs sm:text-sm text-slate-400">
              <li className="font-semibold text-slate-200">Junior College (11th & 12th):</li>
              ...
              <li className="font-semibold text-slate-200 pt-2">Undergraduate Degree:</li>
      ...
      <div className="border-t border-white/10 bg-slate-950/40">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-slate-400 text-center sm:text-left">
          <p>© {new Date().getFullYear()} Shriram Education Society's Shri. Anandrao Pawar College, Chiplun. All rights reserved.</p>
          <p className="text-amber-300 font-medium">Affiliated to University of Mumbai · NAAC Accredited</p>
        </div>
      </div>
    </footer>
  );
